...
atomic operation通常包含几个步骤，要么几个步骤都执行完毕，要么一个步骤都不执行。步骤之间没有空档，不会说一个步骤执行了其他步骤还没执行的情况
...
这个函数貌似是用于处理很多奇怪设备的I/O函数，这些设备通常没办法使用`read`和`write`进行充分的操作。各种设备可以定义自己的request，完成
